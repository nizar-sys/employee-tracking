            ...
            <div class="card-header border-bottom">
                ...
                <div class="d-flex justify-content-start align-items-center row gx-5 pt-4 gap-5 gap-md-0">
                    ...
                    <div class="col-md-4 status_filter">
                        <select id="status_filter" class="form-select" data-filter="status"
                            name="status_filter">
                            <option value="">All Status</option>
                            @foreach ($attendanceStatuses as $attendanceStatus)
                                <option value="{{ $attendanceStatus }}">{{ ucfirst($attendanceStatus) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script>
        var urlDelete = "{{ route('attendances.destroy', ':id') }}";
    </script>
